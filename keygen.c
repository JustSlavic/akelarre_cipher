...
void gen_decrypt_keys(uint16_t* master_key, uint32_t* d, uint32_t nrounds) {
    ...
    d[0] = (uint32_t) -k[4 + 13*nrounds + 1];
    d[1] =  k[4 + 13*nrounds + 2];
    d[2] =  k[4 + 13*nrounds + 3];
    d[3] = (uint32_t) -k[4 + 13*nrounds + 4];

    for (int r = 0; r < nrounds; ++r) {
        d[13*r+4] = (uint32_t) -k[4 + 13*(nrounds-r)];
        for(int j = 5; j <= 16; ++j){
            d[13*r+j] = k[13*(nrounds - r - 1) + j];
        }
    }

    d[13*nrounds + 4] = (uint32_t) -k[4];

    d[13*nrounds + 5] = (uint32_t) -k[0];
    d[13*nrounds + 6] =  k[1];
    d[13*nrounds + 7] =  k[2];
    d[13*nrounds + 8] = (uint32_t) -k[3];
}
